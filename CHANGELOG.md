...
### Fixed
- **Tool return format**: All tools now return the standard Strands `ToolResult`
  format: `{"status": "success"|"error", "content": [{"text": ...}, {"json": ...}]}`.
  Previously returned a non-standard `{"status": ..., "message": ..., **extra}` dict
  that agents couldn't properly parse.

### Added
- **Inline image output**: Dense tasks (seg, normal, albedo, pointmap) and pose
  now return visualization images as inline `{"image": {"format": ..., "source": {"bytes": ...}}}`
  content blocks - same format as `strands_tools.image_reader`. The agent can
  _see_ the output without a separate `image_reader` call.
- **`ok_with_images()` helper** in `_common.py` for building responses with
  embedded image content blocks (up to 5 images per response).
- **PyPI publish workflow** (`.github/workflows/publish.yml`) - triggers on
  GitHub release, runs tests, publishes to PyPI via `PYPI_API_TOKEN` secret.

### Changed
- License changed from Proprietary to **MIT** for public release.
- README rewritten for PyPI: install via `pip install strands-sapiens`,
  removed private-repo references, added response format docs and badges.
- `__version__` synced to `0.1.2` across `pyproject.toml` and `__init__.py`.
