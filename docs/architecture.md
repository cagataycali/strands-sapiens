...
```
strands_sapiens/
├── __init__.py       # re-exports the 7 @tool functions + TOOLS list
...
```mermaid
flowchart LR
    A[Image path] --> B[RTMDet-m detector<br/>person bboxes]
    B --> C[Crop per person]
...
- **Video mode**: a `sapiens_video(input_mp4, ...)` tool using temporal consistency between frames.
