...
<figure markdown="span">
  ![Input → Segmentation → Normals](assets/images/examples/hero_triptych_sm.jpg){ width="100%" loading="lazy" }
  <figcaption>Real output from <code>sapiens_seg</code> and <code>sapiens_normal</code> on a single image (0.4b model, NVIDIA Thor)</figcaption>
</figure>
...
<figure markdown="span">
  ![Tool suite overview](assets/diagrams/hero-tools.svg){ width="100%" loading="lazy" }
</figure>
...
-   **🎬 Video processing**
    ...
    ```python
    sapiens_video(video_path="dance.mp4", output_dir="out/", task="seg")
    ```

    → [Video guide](guide/video.md)
