...
def test_sapiens_info_runs(monkeypatch, tmp_path):
    ...
    assert "checkpoint_root" in result
    assert result["checkpoint_root_exists"] is False
    assert result["available"] == {}
    assert result["detector_present"] is False
    assert "cuda" in result
    assert "available" in result["cuda"]
    assert "device_count" in result["cuda"]


def test_ok_err_format():
    from strands_sapiens._common import err, ok
    r = ok("hi", foo=1)
    assert r == {"status": "success", "message": "hi", "foo": 1}
    r = err("bad", code=2)
    assert r == {"status": "error", "message": "bad", "code": 2}
